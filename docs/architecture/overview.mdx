...
![Indexes of vegetables table](/img/docs/vegetables_indexes.svg)
...
![Dual pointers scheme](/img/docs/dual_pointers.svg)
...
See [concurrency algorithms in OrioleDB B-tree](/docs/architecture/concurrency) for details.
...
![Page structure](/img/docs/page_structure.svg)
...
![Copy-on-write checkpoint 1](/img/docs/cow_1.svg)
...
![Concurrent checkpoint 1](/img/docs/checkpoint_concurrent_1.svg)
...
![Concurrent checkpoint 2](/img/docs/checkpoint_concurrent_2.svg)
...
![Concurrent checkpoint 3](/img/docs/checkpoint_concurrent_3.svg)

Then, checkpointer finished writing page images `7*`, `3*` and `1*` to checkpoint 2.

![Concurrent checkpoint 4](/img/docs/checkpoint_concurrent_4.svg)
...
At the moment of time, there could be multiple checkpoints which use different but overlapping sets of blocks. Therefore, [free space management](/docs/architecture/fsm) becomes an untrivial task.

See [the detailed description of checkpointing algorithm](/docs/architecture/checkpoints).
...
![Row level undo](/img/docs/row_level_undo.svg)
...
![Block level undo](/img/docs/block_level_undo.svg)
...
OrioleDB uses both circular buffers and block buffers for accessing the undo log. See [undo log storage](/docs/architecture/buffering) for details.
...
![Block level undo](/img/docs/recovery_wokers.svg)
...
See [the details about OrioleDB's recovery](/docs/architecture/recovery).
